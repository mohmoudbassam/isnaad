...
class DiscountController extends Controller
{
    public function index()
    {
        ...
        return view('m_design.finance.discount.index', $data);
    }

    public function list(Request $request)
    {
        $invoicies = invoice_discount::query()->with(['store']);
        ...
        return DataTables::of($invoicies)
            ->addColumn('actions', function ($item) {
                ...
                $edit = '<a class="dropdown-item" onclick="showModal( \'' . route('discount-form', ['id' => $item->id]) . '\'  )">
    ...
    public function delete(Request $request){
        invoice_discount::find($request->id)->delete();
