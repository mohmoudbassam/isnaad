...
class FDA {
    ...
    public static function authorization()
    {
        ...
        try {
            ...
            if ($result->status == 'success') {
                ...
                $FDA = carrier::where('name', 'FDA')->first();
                $FDA->token = $id_token;
                $FDA->expires_at = strtotime('+1 day');
                $FDA->save();
                return $id_token;
            } else {
                Log::error('error generating token for FDA');
                return 0;
            }
        } catch (\Exception $e) {
            Log::error('error generating token for FDA ' . $e->getMessage());
            return 0;
        }
    }
    ...
    public static function send_request($data, $end_point, $request_type, $token = 1)
    {
        ...
        if ($token == 1) {
            $FDA = carrier::where('name', 'FDA')->first();
            if ($FDA->expires_at > time()) {
                $header['Authorization'] = 'Bearer ' . $FDA->token;
            } else {
        ...
        try {
            ...
            Log::error('error in send request for FDA ' . $exception->getMessage());
            return 0;
        }
    }
    ...
    public static function create_shipment($order)
    {
        ...
        if ($result) {
            ...
            if ($result->status == "success") {
                ...
                Log::alert("Error from FDA : " . $order->MLVID . ' ' . json_encode($result->message));
                return $data;
            }
        } else {
            $data = [
            ...
            Log::alert("Error Adding Shipment To FDA: " . $order->MLVID);
            return $data;
        }
    }
    ...
    public static function create_return_shipment($order)
    {
        ...
        if ($result) {
            ...
            if ($result->status == "success") {
                ...
                Log::alert("Error from FDA : " . $order->order->order_number . ' ' . json_encode($result->message));
                return $data;
            }
        } else {
            $data = [
