...
class Jones
{
    ...
    static $secret_key = 'ab7abf-01215b-2c270f-3ff22b-40d890';
    static $customerId = '10070150';
...
//dd($result);
        if ($result) {
            if (isset($result->awb_no)) {
                ...
                $data = [
                    'msg' => "Error from Jones " . json_encode($result->status),
                    'status' => 'error'
                ];
                return $data;
            }
            ...
            $data = [
                'msg' => "Error adding order to Jones",
                'status' => 'error'
            ];
            return $data;
        }
    ...
    public static function update_status($tracking_number, $id)
    {
        ...
        if ($result) {
            ...
            //dd($result->travel_history[$count - 1]->code);
            if ($result->travel_history[$count - 1]->code == 'POD' || $result->travel_history[$count - 1]->new_status == 'Delivered') {
               // dd(456,$result);
                $order = order::find($id);
                $order->order_status = 'Delivered';
                $newdate= date("Y-m-d", strtotime($result->travel_history[$count - 1]->entry_date));
               // dd($newdate);
                $order->delivery_date = $newdate;
                $order->Last_Status = $result->travel_history[$count - 1]->new_status;
                $order->save();
            } elseif ($result->travel_history[$count - 1]->new_status == 'Return'|| $result->travel_history[$count - 1]->new_status == ' Cancel') {
                $order = order::find($id);
               ...
               // dd(123,$result->travel_history[$count - 1]->new_status);
                $order = order::find($id);
               $order->Last_Status = $result->travel_history[$count - 1]->new_status;
                $order->save();
            }
