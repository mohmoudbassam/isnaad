...
class Lastpoint extends update_stores{
    static $username = 'I.ansari@isnaad.sa';
    static $password = 'Info@123';
    ...
    public static function authorization()
    {
        ...
        try {
            ...
            if ($result->status == 'success') {
                ...
                $mkhdoom = carrier::where('name', 'Lastpoint')->first();
                $mkhdoom->token = $id_token;
                $mkhdoom->expires_at = strtotime('+1 day');
                $mkhdoom->save();
                return $id_token;
            } else {
                Log::error('error generating token for lastpoint');
                return 0;
            }
        } catch (\Exception $e) {
            Log::error('error generating token for lastpoint ' . $e->getMessage());
            return 0;
        }
    }
    ...
    public static function send_request($data, $end_point, $request_type, $token = 1)
    {
        ...
        try {
            ...
            Log::error('error in send request for lastpoint ' . $exception->getMessage());
            return 0;
        }
    }
...
$charge_items[] = array(
        'charge_type' => 'cod' ,
        'paid'=>$order->CODamount == 0 ? true:false,
        'charge' => $order->CODamount > 0 ? $order->CODamount:0,
        'payer' => 'recipient'
    );
    $charge_items[] = array(
        'charge_type' => 'service_custom',
        'charge' => 0,
        'payer' =>  $order->CODamount > 0 ?'recipient' : 'sender'
    );
        $data = [
            'sender_data' => [
        ...
        if ($result) {
            ...
            if ($result->status == "success") {
                $data = [
                    ...
                    'waybill_url' => route('LastpointLabel',array('tracking_num' =>$result->data->order_number)),
                    'status' => 'success',
                ...
                $data = [
                    'msg' => "Error from Lastpoint " . json_encode($result->message),
                    'status' => 'error'
                ];
                Log::alert("Error from Lastpoint : " . $order->MLVID . ' ' . json_encode($result->message));
                return $data;
            }
        } else {
            $data = [
                'msg' => "Error adding order to Lastpoint",
                'status' => 'error'
            ];
            Log::alert("Error Adding Shipment To Lastpoint: " . $order->MLVID);
            return $data;
        }
    }
      ...
      // dd($result);
        if ($result) {
            ...
            if ($result->status == "success") {
                ...
                Log::alert("Error from Lastpoint : " . $order->order_number . ' ' . json_encode($result->message));
                return $data;
            }
        } else {
            $data = [
            ...
            Log::alert("Error Adding Shipment To Lastpoint: " . $order->order_number);
            return $data;
        }
    }
        ...
        if(isset($result->status)){
            if ($result->status == "success") {
                ...
                if($result->data->list[0]->status=="completed"){
                    ...
                    Log::alert("deliverd Lastpoint: " . $id);
                   ...
                   $newDate = date("Y-m-d", strtotime($originalDate));
                     $order->update([
                         'Last_Status'=>$result->data->list[0]->status,
                         'return_date_carrier'=> $originalDate]);
                    
                    
                    Log::alert("Returned Lastpoint: " . $id);
                }else{
                    $order=order::find($id);
                    $order->update(['order_status'=>'inTransit','Last_Status'=>$result->data->list[0]->status]);
                    Log::alert("inTransit Lastpoint: " . $id);
                }
            } else {
