...
class Sadatalbukhur_cron extends Command
{
	use Salla_helper;
    /**
     ...
     * @var string
     */
    protected $signature = 'Sadatalbukhur:cron';
     ...
     * @var string
     */
    protected $description = 'this crone job for Sadatalbukhur update status';
    ...
    public function handle()
    {
       ...
       Log::error('Sadatalbukhur cron job');
        ...
        foreach ($orders as $order) {
            if ($order->order_status == 'inTransit' || $order->order_status == 'Data Uplouded') {
                ...
                if ($order->in_transit != null) {
                  ...
                  $statusCode = $this->sendRequest($order);
                if ($statusCode == 200) {
                     Log::error('suc Sadatalbukhur orNum' . $order->order_number);
