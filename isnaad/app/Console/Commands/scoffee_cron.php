...
class scoffee_cron extends Command
{
	use Salla_helper;
    /**
     ...
     * @var string
     */
    protected $signature = 'scoffee:cron';
     ...
     * @var string
     */
    protected $description = 'this crone job for scoffee update status';
    ...
    {
       Log::error('scoffee cron job');

        $orders = order::where([['store_id', '7'], ['processing_status', '0'], ['active', '1']])
            ->doesnthave('deliverd_orders')->get();
          
       $orders= $orders->filter(function ($order){
       return (strlen($order->order_number) >= 6);
    });
